...
| 任务书条目 | 类型 | 本项目计划 |
|------------|------|------------|
| 知识库 ≥15 个本校场馆 `.md` 文件 | 必做 | 当前 2 个示例，待补至 15 个 |
...
4. 在 `knowledge/` 录入本校场馆资料，共 **≥15 个** `.md` 文件（每馆一份）；`indexer` 再按 `##` 切块供 RAG。  
...
| 知识库文件过少 | 答辩前 `knowledge/` 须 **≥15 个** md，可先 5–8 个再补 |
...
- 确认 `knowledge/` 下 **`.md` 文件数 ≥15**（任务书要求）。  
- `indexer` 输出的 **切块段数** 可多于 15，仅用于检索，与文件个数分开统计。  
...
- `knowledge/` 中 **≥15 个** `.md`；执行 `indexer.py` 后 `chunks.json` 非空，每条可追溯到源文件。
...
| 周次 | 阶段 | 里程碑 |
|------|------|--------|
| 第 1 周 | P0 + P1 | DeepSeek 通；knowledge ≥15 个 md |
